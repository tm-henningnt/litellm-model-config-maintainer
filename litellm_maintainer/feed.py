...
class Offering:
    ...
    def canonical_model_id(self) -> str | None:
        """The Canonical Model this Offering serves, or `None`.

        Several Offerings share one Canonical Model: the audited Feed
        reaches `minimax/minimax-m3` through five providers. A Declared
        Offering names this value to say "the same model", which is how a
        Reference Model works. See CONTEXT.md, "Reference Model".
        """
        return (self.raw.get("canonical_model") or {}).get("id")
...
class Feed:
    ...
    def offerings_for_canonical_model(self, canonical_model_id: str) -> tuple[Offering, ...]:
        """Every Offering the Feed publishes for one Canonical Model.

        Reads the whole document, ignoring Policy and Selection. A
        Reference Model is usually an Offering this proxy does NOT serve
        — a paid mirror a pricing filter removed — and its numbers are
        wanted anyway. See CONTEXT.md, "Reference Model".
        """
        return tuple(
            o for o in self.offerings if o.canonical_model_id == canonical_model_id
        )
