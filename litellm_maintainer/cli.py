...
def _confirming_worklist(
    ...
    *,
    ...
    - A self-identifying failure needs no Probe. `classify` already read
      ...
      already refusing us.
    - An ambiguous failure (`inconclusive`) gets exactly one Probe, for
      that one Offering. One ambiguous call must never change Health
      State on its own.
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    Read-only: writes no file and makes no provider call. The one network
    call is the proxy liveliness check, which is the same check `run`
    already makes. Exit 0 when every check passed, 1 when any failed, so
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    stale = staleness_warning(
        ...
        maximum_age_hours=policy.feed.maximum_age_hours if policy.feed else 24.0,
        now=now,
    )
    if stale is not None:
        print(redact(f"warning: {stale}", mapping))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    status_parser.add_argument("--feed", required=True, help="Path to the Feed document")
    status_parser.add_argument("--policy", required=True, help="Path to the Policy file")
