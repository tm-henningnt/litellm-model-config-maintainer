...
def validate_config_before_write(
    ...
    3. Every credential variable resolves. A credential variable is an
       ...
       `litellm_params`, OR a top-level `master_key` (an operator
       setting, passed through verbatim from Policy's `proxy_settings`
       — see `litellm_maintainer.policy.ProxySettings`).
       `credential_resolver` looks `NAME` up; pass `{}.get` or a fake
       mapping's `.get` in a test, so no test needs a real credential in
       the environment.

       Both `general_settings` and `litellm_settings` are checked.
       litellm reads only `general_settings.master_key`, and that is
       where a Policy must put it. A Policy that still holds the key
       under `litellm_settings` is checked all the same, because the
       operator who wrote it there believes it is the live key.
    ...
    for block in ("general_settings", "litellm_settings"):
        master_key = config.get(block, {}).get("master_key")
        ...
        if credential_resolver(name) is None:
            problems.append(
                f"{block}.master_key references credential variable "
