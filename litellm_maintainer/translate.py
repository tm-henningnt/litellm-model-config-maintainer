...
# The Qwen Token Plan speaks two protocols (see the Feed's own
# `api_protocols` for `qwencloud-token-plan`): the OpenAI-compatible
# route the Feed's `endpoint.base_url` states, and an
# Anthropic-compatible route at a different base URL. Direct testing
# showed the Anthropic route works and needs no special-casing beyond
# this rule; switching to the Feed's stated OpenAI route is out of
# scope (see the spec, "Out of Scope"). `api_base` is a fixed
# environment variable reference, not the Feed's `base_url`, because
# the two routes live at different URLs.
QWEN_TOKEN_PLAN_ANTHROPIC_BASE_URL_VAR = "QWEN_TOKEN_PLAN_ANTHROPIC_BASE_URL"


def qwencloud_token_plan_anthropic(
    offering: Offering, provider: Provider | None
) -> dict[str, Any]:
    """The Qwen Token Plan rule: Anthropic-shaped calling convention."""
    credential_variable = _credential_variable(offering, provider)
    return {
        "model": f"anthropic/{offering.provider_model_id}",
        "api_base": f"os.environ/{QWEN_TOKEN_PLAN_ANTHROPIC_BASE_URL_VAR}",
        "api_key": f"os.environ/{credential_variable}",
    }
...
TRANSLATION_RULES: dict[str, TranslationRule] = {
    ...
    "qwencloud-token-plan": qwencloud_token_plan_anthropic,
}
