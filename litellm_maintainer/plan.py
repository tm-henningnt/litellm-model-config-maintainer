...
# Why Selection dropped one Discovered Offering. `PlanReport.dropped`
# maps an Offering id to exactly one of these, and `explain` turns it
# into a sentence naming the construct responsible.
#
# Stable tokens a caller may branch on. `plan` records the FIRST reason
# that applies, because Selection stops at the first one: an Offering
# barred by two filters names the filter that ran first.
DROPPED_PROVIDER_NOT_CONFIGURED = "provider_not_configured"
DROPPED_NO_TRANSLATION_RULE = "no_translation_rule"
DROPPED_UNTRANSLATABLE = "untranslatable_offering"
DROPPED_VISIBILITY = "feed_visibility"
DROPPED_BASELINE = "feed_baseline"
DROPPED_PROVIDER_MODELS = "provider_models"
DROPPED_PROVIDER_PRICING = "provider_pricing"
DROPPED_PLAN_EDITION = "provider_plan_edition"
DROPPED_WITHHELD = "withheld"
DROPPED_SUPERSEDED = "superseded_by_declared"
DROPPED_GONE = "gone"
DROPPED_UNSCORED = "unscored_candidate"
...
class PlanReport:
    ...
    `excluded` lists Offering ids Health State marks excluded. They
    STAY in the Generated Config and a caller can still reach them; they
    stop being recommended. `unlisted` lists Offering ids left out of
    the Generated Config because Health State records them Gone. A
    Withheld Offering is Unlisted too and is reported in `withheld`,
    which names the operator's own reason.

    A write to the Generated Config restarts the proxy and ends every
    session in flight, so only a deliberate or terminal fact may drive
    one — see ADR 0014. An Offering can therefore appear in `admitted`
    and in `excluded` at once, which is correct: it is written into the
    file and it is known not to answer.
...
def _is_unlisting(record: OfferingHealth) -> bool:
    """State whether this Health State record Unlists the Offering.

    Only a terminal fact does. Every write to the Generated Config
    restarts the proxy and ends the sessions in flight, so a measurement
    that can change its mind must never drive one — see ADR 0014.

    `gone` is the one bucket that qualifies. The identifier no longer
    answers for this account, it carries no reset time, and nothing
    clears it but the operator. Every other bucket describes a condition
    that a later Probe can reverse, so the Offering stays written and
    stops being recommended instead.

    Withheld is the other cause of Unlisted, and it never reaches here:
    Policy removes a Withheld Offering before Health State is read.
    """
    return record.bucket == GONE
...
def plan(
    *, feed: Feed, policy: Policy, health: dict[str, OfferingHealth], now: datetime
    ...
    excluded: list[str] = []
    unlisted: list[str] = []
    # Offering id -> the token naming why Selection dropped it.
    ...
    for declared in policy.declared:
        ...
        if record is not None and record.excluded:
            if _is_unlisting(record):
                unlisted.append(declared.alias)
                continue
            # Excluded and still written. The Offering keeps its entry,
            # so a caller reaches the provider's own error instead of
            # "model not found", and no write restarts the proxy for a
            # condition that may clear on the next Probe (ADR 0014).
    ...
    for provider_id, rule in sorted(policy.providers.items()):
        if provider_id not in _translatable_providers():
            ...
            #
            # Record every Offering it would have carried. This drops a
            # whole provider at once and says nothing anywhere else, so
            # without the record `explain` can only report that the
            # Offering is missing, never why.
            for offering in feed.offerings_for(provider_id):
                dropped[offering.id] = DROPPED_NO_TRANSLATION_RULE
            continue
        ...
        for offering in feed.offerings_for(provider_id):
            ...
            if offering.visibility != "listed" and not sunsetting_offering:
                ...
                dropped[offering.id] = DROPPED_VISIBILITY
                continue
            if not _passes_baseline(offering):
                dropped[offering.id] = DROPPED_BASELINE
                continue
            if allowed_ids is not None and offering.id not in allowed_ids:
                dropped[offering.id] = DROPPED_PROVIDER_MODELS
                continue
            if allowed_pricing is not None and offering.pricing_kind not in allowed_pricing:
                dropped[offering.id] = DROPPED_PROVIDER_PRICING
                continue
            if not _passes_plan_edition(offering, rule.plan_edition):
                ...
                # roster changes.
                dropped[offering.id] = DROPPED_PLAN_EDITION
                continue
            if offering.id in policy.withheld:
                ...
                dropped[offering.id] = DROPPED_WITHHELD
                continue
            if offering.id in superseded_ids:
                ...
                dropped[offering.id] = DROPPED_SUPERSEDED
                continue
            ...
            #
            # Excluded no longer removes the Offering. Only a terminal
            # fact does, because every write restarts the proxy (ADR
            # 0014). The Offering falls through to admission and stops
            # being recommended instead.
            is_excluded = False
            if record is not None and record.excluded:
                if _is_unlisting(record):
                    unlisted.append(offering.id)
                    dropped[offering.id] = DROPPED_GONE
                    continue
                is_excluded = True

            if not _admit_offering(offering, policy, candidates):
                dropped[offering.id] = DROPPED_UNSCORED
                continue

            # Recorded only now, because `excluded` states "in the config
            # and not recommended". An unscored Offering stops at the
            # Candidate gate above and never reaches the config, so
            # naming it here would report a served Offering that is not
            # served — the false claim ADR 0014 exists to prevent.
            if is_excluded:
            ...
            try:
                ...
                # The Feed describes this provider too thinly to build a
                # call from. Recorded rather than dropped in silence:
                # the Offering clears every Policy filter and then
                # disappears, which reads as a Policy fault and is not
                # one.
                dropped[offering.id] = DROPPED_UNTRANSLATABLE
                continue
