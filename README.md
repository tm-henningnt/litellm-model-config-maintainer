...
Seven files. Each has exactly one writer, so no process can overwrite
another's work. See
...
| `state/headroom.json` | `headroom refresh` | how drawn each Allowance is, as a source measured it |
...
- **Reports how drawn each Allowance is**, when you point it at a source
  that measures one. This is optional. See below.

### Optional: how much of an Allowance is left

`guidance` and `entitlements` answer "which model, and may I spend?". They
do not answer "how much is left". Nothing in the Feed states that, because
a ceiling belongs to your credential and not to the model.

An external source can measure it. Point the tool at one, and every Route
and every Entitlement carries a **Headroom**: the share drawn, the window
it binds on, and when that window resets.

    litellm-maintainer headroom refresh --env <env file>

This is off by default. Omit `headroom` from your Policy and no Reading is
taken, no seventh file appears, and every published Headroom reads `null`.

Two rules bound it, and both matter:

- **A Reading never rewrites the Generated Config.** A drawn Allowance
  changes what `guidance` recommends. It never removes an alias from the
  proxy. Removal stays with the Probe and the Health State.
- **A missing Headroom is not free capacity.** `null` means nobody
  measured it. Read `headroom_state`, never the absence of a figure.

You must state the mapping yourself: which source key measures which
Allowance, and which Offerings draw on which window. The tool infers none
of it, because a wrong guess reads exactly like a right one.
**Verify each mapping against the provider's own dashboard before you
write it.** No check here can catch a source that was wrong from the start.

Read [ADR 0013](./docs/adr/0013-a-headroom-reports-what-a-source-measured.md)
and the Headroom section of
...
The same rule covers the container setup. `docker/compose.yaml` states
no credential. Each service reads an env file from `~/.config/litellm/`,
and `.gitignore` blocks `docker/*.env` so a copy left here is never
tracked.

## Run the proxies in containers

`docker/` holds a compose file that runs the proxy, the ChatGPT seat
workers and the database as containers. It is optional. The host install
in [docs/getting-started.md](./docs/getting-started.md) stays supported.

The maintainer itself does NOT move. It reads Headroom through
`codexbar`, a macOS binary, so it stays on the host under launchd. It
writes the Generated Config to a directory the compose file mounts, and
the proxy reloads on the write.

Containers earn their place for one reason: the two litellm source
patches. On a host install `uv tool upgrade litellm` removes both and
nothing reports the loss. In the image they are a build step that
verifies its own result, so a build fails rather than shipping a proxy
that answers nothing.

The image is stock litellm plus those patches. Provider handler modules
are not baked in: `deploy` writes them beside the Generated Config, and
the compose file mounts that directory, so a handler change needs a
restart and not a rebuild.

Read [docker/README.md](./docker/README.md) before you start it. Two
things there are easy to get wrong. The maintainer's prober calls a
Declared Offering's `api_base` from the host while the proxy calls the
same URL from inside the container network, so the worker hostname must
resolve from both. And `doctor` cannot see inside a container, so the
patch checks are answered by a command the image carries instead.

No secret goes in the compose file. Each service reads its own env file
from `~/.config/litellm/`, outside this repository.
...
18. Ask me whether I run a source that measures how drawn each Allowance
    is. If I do not, stop here. If I do, follow the Headroom section of
    docs/operations.md. Map one Allowance at a time. Show me the source's
    figure, and let me check it against that provider's own dashboard,
    before you write the mapping into my Policy. Never guess a mapping
    from a name.
19. If I ask for a snapshot to read myself, write one with
...
docker/                     optional: the proxies and the database as containers
