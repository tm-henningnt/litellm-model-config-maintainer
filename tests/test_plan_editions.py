...
# The committed Feed, so this roster is deterministic. Never the
# operator's live feed.json: it changes under the test.
OPERATOR_FEED_PATH = Path(__file__).parent / "fixtures" / "feed-current.json"
...
@pytest.mark.skipif(
    not PINNED_POLICY_PATH.exists() or not OPERATOR_FEED_PATH.exists(),
