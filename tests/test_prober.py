...
def test_the_worklist_for_the_pinned_policy_is_pinned():
    """A change to the baseline, or to any other filter, must not move
    these counts silently.

    The counts are read from the code, never guessed: `build_worklist`
    against `fixtures/feed-current.json` and `fixtures/policy-pinned.yaml`,
    with empty Health State. Both inputs are committed, so the numbers
    move only when behaviour moves.

    Re-derive them after any deliberate change to either fixture:
        ...
        w = build_worklist(
            feed=load_feed(Path('tests/fixtures/feed-current.json')),
            policy=load_policy(Path('tests/fixtures/policy-pinned.yaml')),
            health=HealthState(offerings={}),
            now=datetime(2026, 8, 1, tzinfo=timezone.utc))
        print(len(w.targets), len(w.skipped_passthrough))"
    """
    ...
    assert len(worklist.targets) == 79
    ...
    assert len(worklist.skipped_passthrough) == 2
    ...
    # `skipped_passthrough`, never in `targets`. A seat and a private
    # host ARE legitimate non-passthrough Declared targets (`offering`
    ...
    for target in declared_targets:
        ...
        # The rule this test is about. An alias-prefix check here would
        # measure the fixture's naming instead.
...
def test_an_unlisted_alias_takes_its_variant_out_of_the_generated_config():
    """A Client-Facing Variant is the same wire request under a second
    name, so it leaves the Generated Config exactly when the Alias it
    widens does. Leaving one behind offers an Alias certain to fail."""
...
def test_an_excluded_alias_keeps_itself_and_its_variant_offered():
    """An exclusion no longer removes an Alias, so the pair stays whole
    and stops being recommended together (ADR 0014)."""
    from litellm_maintainer.plan import plan
