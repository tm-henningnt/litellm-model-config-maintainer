...
def test_declared_offerings_are_counted_outside_the_entitlement_list():
    """An Entitlement is a relationship with a provider. A Declared Offering has none."""
    view = _entitlement_view(declared=[DIRECT, PASSTHROUGH], admitted=("claude-direct",))

    assert view.declared_answering == 1
    assert [e.provider_id for e in view.entitlements] == ["openrouter"]
