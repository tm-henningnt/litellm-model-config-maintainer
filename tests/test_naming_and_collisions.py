...
def test_every_alias_in_the_frozen_config_is_reproduced_except_the_seven_renamed(
    feed_audited, operator_policy, frozen_config
):
    """User story 14 ("preserve every Alias exactly") was waived on
    2026-07-25 for exactly seven Aliases, so one consistent naming rule
    could replace the hand-built names (spec-corrections.md, correction
    4). Every other Alias currently in use is reproduced unchanged,
    except the nine the operator's Policy now Withholds (Qwen Token
    Plan, personal-tier denial — HTTP 403 on every call). None of the
    remaining 62 is renamed.

    On top of the frozen 78, exactly 13 new Aliases now appear, named
    explicitly rather than waved through by an open-ended check:
    `SEAT_ALIASES` (the 12 ChatGPT worker seats, added 2026-07-26) and
    `HY3_PREVIEW_ALIAS` (one newly-admitted Discovered Offering,
    unrelated to the seat change — see its docstring in
    `test_translate.py`). Nine Aliases are withdrawn, also named
    explicitly: `personal_plan_denied_aliases`, the derived Aliases of
    `PERSONAL_PLAN_DENIED_OFFERING_IDS`.
    """
    result = plan(feed=feed_audited, policy=operator_policy, health={}, now=NOW)
    generated_aliases = {e["model_name"] for e in result.config["model_list"]}

    frozen_aliases = {e["model_name"] for e in frozen_config["model_list"]}
    expected_aliases = {RENAMED_ALIASES.get(alias, alias) for alias in frozen_aliases}
    new_aliases = (
        SEAT_ALIASES
        | CLIENT_FACING_VARIANT_ALIASES
        | derived_variant_aliases(generated_aliases)
    ...
    derived_variants = derived_variant_aliases(generated_aliases)
    ...
    assert denied_aliases <= expected_aliases
    assert new_aliases <= generated_aliases
    assert denied_aliases.isdisjoint(generated_aliases), (
        "a Withheld Alias must never reach the Generated Config"
    )
    assert DIRECT_CHATGPT_ALIASES_RETIRED <= expected_aliases
    assert DIRECT_CHATGPT_ALIASES_RETIRED.isdisjoint(generated_aliases), (
        "a retired direct chatgpt/ Alias must never reach the Generated Config"
    )
    assert (
        generated_aliases - new_aliases
        == expected_aliases - denied_aliases - DIRECT_CHATGPT_ALIASES_RETIRED
    )
    # None of the renamed Aliases survives; each produces its recorded
    # new name instead.
    for old_name, new_name in RENAMED_ALIASES.items():
        assert old_name not in generated_aliases
        assert new_name in generated_aliases
    # Every Alias derives; the override map is empty on purpose
    # (spec-corrections.md, correction 4).
    assert operator_policy.naming.alias_overrides == {}
