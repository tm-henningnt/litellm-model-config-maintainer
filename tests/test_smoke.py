...
from litellm_maintainer.policy import Pacing, load_policy, parse_policy
...
def test_an_excluded_offering_does_not_fail_the_wiring_check_when_a_healthy_sibling_exists():
    entries = (
        SmokeEntry(key="qwen:excluded", alias="claude-qwen-excluded", rule="qwencloud_token_plan_anthropic[qwencloud-token-plan]"),
        SmokeEntry(key="qwen:healthy", alias="claude-qwen-healthy", rule="qwencloud_token_plan_anthropic[qwencloud-token-plan]"),
    )
    health = {
...
def test_a_rule_with_no_healthy_offering_is_unverified_not_failed():
    entries = (
        SmokeEntry(key="qwen:only", alias="claude-qwen-only", rule="qwencloud_token_plan_anthropic[qwencloud-token-plan]"),
    )
...
def test_the_operator_policy_gains_a_smoke_rule_for_the_chatgpt_worker_seats():
    """Determined by running the code, not by guessing: against
    `feed-current.json`, the operator's real Policy produces 8 smoke
    rules.

    One is `declared:openai`, naming the 12 `claude-chatgpt{1,2}-*`
    worker-seat Aliases (`SEAT_ALIASES` in test_translate.py). The seats
    route through the generic `openai/` prefix to a local worker, so they
    group under that label rather than litellm's own `chatgpt/` provider.

    `declared:chatgpt` is deliberately absent. It existed only for the 6
    direct ChatGPT Aliases the operator retired on 2026-07-26
    (`DIRECT_CHATGPT_ALIASES_RETIRED`); every ChatGPT model now goes
    through a seat. A rule with no Offering behind it must not appear.
    """
    ...
    assert "declared:chatgpt" not in grouped
    assert not [e for e in entries if e.alias.startswith("claude-gpt-")]

    assert "declared:openai" in grouped
    seat_entries = grouped["declared:openai"]
    assert len(seat_entries) == 12
    assert all(
        e.alias.startswith(("claude-chatgpt1-", "claude-chatgpt2-")) for e in seat_entries
    )
