...
def test_status_prints_an_excluded_offering_with_its_reason_and_expected_return():
    ...
    excluded_line = next(l for l in lines if l.strip().startswith("qwencloud-token-plan:glm-5"))
...
def test_status_says_plainly_when_an_excluded_offering_has_no_expected_return():
    ...
    excluded_line = next(l for l in lines if l.strip().startswith("groq:some-model"))
...
def test_status_distinguishes_excluded_from_withheld_candidate_and_sunsetting():
    ...
    excluded_section = lines.split("Excluded:")[1].split("Withheld:")[0]
...
def test_offered_counts_and_names_declared_offerings_alongside_discovered(
    ...
    # the operator's pinned number: 74 + 12 ChatGPT worker seats + 3
    # Client-Facing Variants (CLIENT_FACING_VARIANT_ALIASES) - 9
    # personal-plan-denied Qwen Token Plan Offerings - 6 retired direct
    # ChatGPT Aliases (DIRECT_CHATGPT_ALIASES_RETIRED)
    # + one entry per derived Client-Facing Variant.
    assert entry_count == 74 + 12 + 3 - 9 - 6 + len(
        result.report.client_facing_variants
    )
...
def test_withheld_names_every_feed_published_entry_even_one_an_earlier_gate_would_also_drop(
    ...
    # 15 Feed-published Withheld ids. The nine Qwen Token Plan ids left
    # on 2026-07-26; `plan_edition` filters them by Selection now.
    assert len(result.report.withheld) == 15
...
def test_a_stale_withheld_entry_names_an_unknown_provider_differently_from_a_known_one(
    ...
    held two (`gdm:*`) until 2026-07-26 and now holds none, and the
...
def test_withheld_and_stale_withheld_together_account_for_every_policy_line(
    ...
    assert len(operator_policy.withheld) == 15
...
def test_a_gone_offering_is_reported_with_advice_to_remove_it_from_policy():
    ...
    gone_line = next(l for l in lines if l.strip().startswith("acme:retired-model"))
