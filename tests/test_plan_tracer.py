...
operator's real Policy at `/Users/hentol/.config/litellm-maintainer/
policy.yaml`. Both are read-only inputs; this test writes nothing to
either.
"""
...
from test_translate import HY3_PREVIEW_ALIAS
...
def test_generated_opencode_go_entries_match_the_frozen_config_with_one_difference(
    frozen_opencode_go_entries, generated_entries
):
    """Every generated OpenCode Go entry matches the frozen config.

    The one intended difference: the seven Aliases in
    `ANTHROPIC_PREFIXED_ALIASES` move from the `anthropic/` prefix and
    the `/messages` base URL to the generic `openai/` prefix and the
    plain OpenCode Go base URL. Every other field — every Alias, every
    other `litellm_params` — matches exactly.

    One further Alias, `HY3_PREVIEW_ALIAS`, is not in the frozen config
    at all: `opencode-go:hy3-preview` scores 58.8, above the quality
    threshold, and the operator's Policy sets `opencode-go: mode: all`,
    so it is freshly admitted. This is unrelated to ticket 05's rule and
    is named on its own rather than folded into the "one difference"
    the frozen config already accounts for.
    """
    generated_opencode_go = {
        name: params
        for name, params in generated_entries.items()
        if name.startswith("claude-opencode-go-")
        # A Client-Facing Variant is a second Alias for an entry already
        # counted here, not a further Offering. See ADR 0007.
        and not name.endswith("[1m]")
    }

    assert HY3_PREVIEW_ALIAS in generated_opencode_go
    new_opencode_go_aliases = {HY3_PREVIEW_ALIAS}

    # All 22 named Offerings minus the 2 Withheld (mimo-v2-omni,
    # mimo-v2-pro) leaves 20 admitted OpenCode Go Aliases, matching the
    # 20 active (non-commented) OpenCode Go blocks in the frozen config,
    # plus the one newly-admitted `HY3_PREVIEW_ALIAS` above.
    assert len(generated_opencode_go) == 20 + len(new_opencode_go_aliases)
    assert set(generated_opencode_go) - new_opencode_go_aliases == set(frozen_opencode_go_entries)

    for model_name, generated_params in generated_opencode_go.items():
        if model_name in new_opencode_go_aliases:
            assert generated_params["model"] == "openai/hy3-preview"
            assert generated_params["api_base"] == OPENCODE_GO_BASE_URL
            continue

        frozen_params = frozen_opencode_go_entries[model_name]

        if model_name in ANTHROPIC_PREFIXED_ALIASES:
            assert frozen_params["model"].startswith("anthropic/"), model_name
            assert frozen_params["api_base"].endswith("/messages"), model_name
            expected_provider_model_id = frozen_params["model"].removeprefix("anthropic/")
            assert generated_params["model"] == f"openai/{expected_provider_model_id}", model_name
            assert generated_params["api_base"] == OPENCODE_GO_BASE_URL, model_name
        else:
            assert frozen_params["model"].startswith("openai/"), model_name
            assert generated_params["model"] == frozen_params["model"], model_name
            assert generated_params["api_base"] == frozen_params["api_base"], model_name

        assert generated_params["api_key"] == frozen_params["api_key"], model_name
