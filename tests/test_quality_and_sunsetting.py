...
at `/Users/hentol/.config/litellm-maintainer/policy.yaml`, both
read-only.
"""
...
def test_a_sunsetting_offering_that_stops_answering_is_excluded_by_the_ordinary_path():
    ...
    assert "opencode-go:sunsetting-then-fails" in result.report.excluded
...
def test_an_excluded_unscored_offering_is_reported_as_excluded_not_as_a_candidate():
    feed = _feed_with(
        _offering_raw(
    ...
    assert result.report.excluded == ("opencode-go:excluded-unscored",)
    assert result.report.candidates == ()
