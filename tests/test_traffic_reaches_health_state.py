...
def test_a_proxy_recorded_failure_on_a_discovered_offering_excludes_it_from_the_next_config():
    ...
    # The failure reaches Health State and demotes the Offering. It does
    # not remove it: a gateway error can clear on the next Probe, and a
    # write here would restart the proxy for it (ADR 0014).
    ...
    assert "openrouter:widget" not in result.report.unlisted
    names = [e["model_name"] for e in result.config["model_list"]]
    assert sorted(names) == ["claude-openrouter-gadget", "claude-openrouter-widget"]
