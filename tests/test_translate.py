...
# (`os.environ/LITELLM_CHATGPT_SEAT{1,2}_WORKER_KEY`), so the Prober
...
def derived_variant_aliases(generated) -> set[str]:
    ...
    return {
        ...
        if name.endswith(VARIANT_SUFFIX) and name not in CLIENT_FACING_VARIANT_ALIASES
    }
...
def test_declared_offerings_pass_through_verbatim(operator_policy, feed_audited):
    ...
    # 4 direct-Claude entries, 3 Client-Facing Variants of them
    # (CLIENT_FACING_VARIANT_ALIASES), plus the 12 ChatGPT worker seats
    # (SEAT_ALIASES). The 6 direct `chatgpt/` entries were retired: see
    # DIRECT_CHATGPT_ALIASES_RETIRED.
    assert len(operator_policy.declared) == 4 + 3 + 12
    assert CLIENT_FACING_VARIANT_ALIASES <= set(entries)
    assert not [d for d in operator_policy.declared if d.litellm_params["model"].startswith("chatgpt/")]
...
def test_the_twelve_chatgpt_worker_seats_pass_through_with_the_worker_key_unrewritten(
    ...
    assert len(SEAT_ALIASES) == 12
    assert SEAT_ALIASES <= set(entries)

    for seat, port in (("chatgpt1", "4011"), ("chatgpt2", "4012")):
        worker_key_env = f"os.environ/LITELLM_CHATGPT_SEAT{seat[-1]}_WORKER_KEY"
        for model in SEAT_MODELS:
            alias = f"claude-{seat}-{model}"
            entry = entries[alias]
            params = entry["litellm_params"]
            assert params["model"] == f"openai/claude-{model}"
            assert params["api_base"] == f"http://127.0.0.1:{port}/v1"
            assert params["api_key"] == worker_key_env
            # The operator declares one measured Stated Limit for every
            # seat, identical across both seats because they share a model
            # string. Measured 2026-07-27: the ChatGPT subscription backend
            # accepted 369,603 tokens and refused about 398,000, so this is
            # deliberately BELOW litellm's 1,050,000 for the OpenAI API
            # model, which is a different backend.
            assert entry["model_info"] == {"max_input_tokens": 350000}


# --- The whole-config test ---------------------------------------------


def test_the_operator_policy_reproduces_78_aliases_with_four_differences_and_four_net_additions(
    feed_audited, operator_policy, frozen_config
):
    """`plan`, given the audited snapshot and the operator's Policy,
    reproduces the frozen config's 78 Aliases, with exactly the four
    intended differences `spec-corrections.md` records:

    1. The seven OpenCode Go Aliases in `OPENCODE_GO_MOVED_ALIASES` move
       from the anthropic-shaped prefix to the generic `openai/` prefix
       (ticket 05).
    2. Cost metadata is absent (ticket 09's scope, not this one's).
    3. The seven Aliases in `RENAMED_ALIASES` change to the mechanical
       derivation.
    4. The nine Aliases in `PERSONAL_PLAN_DENIED_OFFERING_IDS` are gone:
       the operator's Policy now Withholds them (personal-plan Qwen
       Token Plan models, HTTP 403 on every call). They are the ONLY new
       removals, named explicitly here — never an open-ended "and
       anything else is fine".

    A fifth gap used to remain and no longer does: the nine
    `claude-cline-free-*` Aliases generated an `openai/` prefix instead of
    `cline/`, because the audited snapshot carries no response envelope
    key for any Offering at all. The operator's
    `providers.cline.response_envelope_key` supplies what the snapshot
    omits, so those nine now reproduce the frozen prefix and this test
    compares them like any other Alias.

    On top of the 78, 13 new Aliases appear (the 12 ChatGPT worker-seat
    Declared Offerings in `SEAT_ALIASES`, added 2026-07-26, and one
    newly-admitted Discovered Offering, `HY3_PREVIEW_ALIAS`), 9 are
    withdrawn (the nine Withheld Qwen Token Plan Aliases) and 6 are
    retired (`DIRECT_CHATGPT_ALIASES_RETIRED`), for a net count of
    78 + 13 - 9 - 6 = 76.
    """
    result = plan(feed=feed_audited, policy=operator_policy, health={}, now=NOW)
    generated = {e["model_name"]: e["litellm_params"] for e in result.config["model_list"]}

    frozen = {e["model_name"]: e["litellm_params"] for e in frozen_config["model_list"]}
    ...
    retired = DIRECT_CHATGPT_ALIASES_RETIRED
    ...
    # The denied and the retired Aliases were both reproduced in the
    # frozen config (they once worked, and the operator once declared
    # them); the entry-by-entry comparison below must not expect either.
    assert denied_aliases <= set(frozen)
    assert retired <= set(frozen)
    assert denied_aliases.isdisjoint(new_aliases)
    assert retired.isdisjoint(new_aliases)

    # +3: CLIENT_FACING_VARIANT_ALIASES. +1: UNWITHHELD_OPENROUTER_ALIAS.
    ...
    assert retired.isdisjoint(set(generated))
    assert set(generated) - new_aliases == set(frozen) - denied_aliases - retired

    for model_name, generated_params in generated.items():
        if model_name in new_aliases:
            continue
        if model_name in OPENCODE_GO_MOVED_ALIASES:
            continue
        frozen_params = frozen[model_name]
        assert generated_params.get("model") == frozen_params.get("model"), model_name
        assert generated_params.get("api_base") == frozen_params.get("api_base"), model_name


def test_no_alias_translates_to_a_different_model_than_the_frozen_config(
    feed_audited, operator_policy, frozen_config
):
    """No Alias may differ from the frozen config in its `model`.

    Nine Cline Aliases used to differ: the audited snapshot declares no
    response envelope key, so they fell back to the generic prefix. The
    operator's `providers.cline.response_envelope_key` closes that, and
    this asserts the closure — a regression in either the Policy key or
    the translation now fails here.
    """
    result = plan(feed=feed_audited, policy=operator_policy, health={}, now=NOW)
    generated = {e["model_name"]: e["litellm_params"] for e in result.config["model_list"]}
    frozen = {e["model_name"]: e["litellm_params"] for e in frozen_config["model_list"]}
    ...
    mismatched_model_prefix = {
        name
        for name, params in generated.items()
        if name not in OPENCODE_GO_MOVED_ALIASES
        and name not in new_aliases
        and params.get("model") != frozen[name].get("model")
    }
    assert mismatched_model_prefix == set()
